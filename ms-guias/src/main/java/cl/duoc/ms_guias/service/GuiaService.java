...
public class GuiaService {
    ...
    // Descargar guía (con validación de permisos: transportista debe coincidir)
    public byte[] descargarGuia(String guiaId, String transportistaSolicitante) throws IOException {
        Guia guia = guiaRepository.findById(guiaId)
                .orElseThrow(() -> new RuntimeException("Guía no encontrada"));
        if (!guia.getTransportista().equals(transportistaSolicitante)) {
            throw new SecurityException("No tiene permiso para descargar esta guía");
        }
