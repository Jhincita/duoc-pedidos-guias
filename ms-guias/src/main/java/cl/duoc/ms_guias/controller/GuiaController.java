...
public class GuiaController {
    ...
    // Subir guía ya generada (si se requiere manual) -> ya se hace en creación, pero puedes exponer un endpoint aparte

    // Descargar guía con validación de permisos (transportista mediante header)
    @GetMapping("/{id}/descargar")
    public ResponseEntity<byte[]> descargarGuia(@PathVariable String id,
                                                @RequestHeader("X-Transportista") String transportista) throws IOException {
    ...
    public ResponseEntity<List<Guia>> obtenerTodas() {
        return ResponseEntity.ok(guiaService.consultarGuias(null, null, null));
    }
}